...
async function fetchLoraMetadata(force = false) {
    ...
    loraFetchOngoing = (async () => {
        ...
        try {
            ...
            const url = "/comfyui-ckpt-utils/lora-cycler-metadata" + (force ? "?refresh=true" : "");
        ...
        if (nodeData.name === "Lora Cycler") {
            ...
            nodeType.prototype.onNodeCreated = function () {
                ...
                this.addWidget("button", "refresh_db", "Refresh Database", async function() {
                    const mw = self.widgets.find(w => w.name === "total_matching_loras");
                    ...
                    try {
                        await fetchLoraMetadata(true);
                        const updateAll = function() {
                            const mw2 = self.widgets.find(function(w) { return w.name === "total_matching_loras"; });
                            if (mw2 && loraCyclerMetadata) {
